        ...
        if self.in_count % 100 == 0:
            self.log.info(
                'In message: 本次入队{}次，当前队列总数{}个, 解析到url个数{}，待入队个数{}，set个数{}，入队总个数{}'
                    .format(c, self.url_q.qsize(), len(urls), len(urls_waiting), len(self.ids_dateset), self.in_count))
...
def url_out(father_url):
    ...
    while wy.out_count%1000 != 0:
        ...
        # 跟帖小于30的，不保存
        if tcount < 100:
            wy.log.info('tcount error: tcount:{} url: {}'.format(tcount,url))
            continue
        try:
            ...
            continue
        if wy.out_count % 2 == 0:
            wy.log.info("out message: 出队第{}次，跟帖：{}个，url：{}".format(wy.out_count,tcount,url))
        wy.out_count += 1
